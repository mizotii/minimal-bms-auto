import pytest
from parse import calculate_beats, map_beats_to_bpm, calculate_time_anchors


class TestCalculateBeats:
    def test_measure_zero_starts_at_beat_zero(self):
        result = calculate_beats(0, {})
        assert result[0] == 0.0

    def test_default_measures_are_four_beats(self):
        result = calculate_beats(2, {})
        assert result[0] == 0.0
        assert result[1] == 4.0
        assert result[2] == 8.0
...
class TestCalculateTimeAnchors:
    ...
    def test_bpm_change_at_beat_zero_replaces_initial(self):
        ...
        result = calculate_time_anchors({0.0: 180.0}, 120.0)
        assert len(result) == 1
    ...
    def test_two_sequential_bpm_changes(self):
        ...
        result = calculate_time_anchors({4.0: 60.0, 8.0: 120.0}, 120.0)
        assert len(result) == 3
    ...
    def test_anchors_are_ordered_by_beat(self):
        ...
        result = calculate_time_anchors({8.0: 90.0, 4.0: 60.0}, 120.0)
